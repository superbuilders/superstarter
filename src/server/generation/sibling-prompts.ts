...
const SUB_TYPE_BRUTAL_ANCHORS: Record<SubTypeId, string> = {
	"verbal.antonyms":
		"Brutal-tier anchor for verbal.antonyms: Low-frequency vocabulary at college+ register (Latin/Greek-rooted, often polysemous), where the dominant sense and a secondary sense have different antonyms — distractors include the antonym of the WRONG sense, plus near-synonyms of the target. Distinguishing requires recognizing which meaning is operative from limited context. Example shape: target = SANGUINE (cheerfully optimistic OR ruddy-complexioned); distractors = pallid (antonym of secondary sense), despondent (antonym of dominant sense — correct), ardent (near-synonym), phlegmatic (related but distinct sense), ruddy (synonym of secondary sense).",
	"verbal.analogies":
	...
	"numerical.ratios":
		"Brutal-tier anchor for numerical.ratios: Three-part ratios (a:b:c) requiring proportional split across all three parts simultaneously, OR ratio-change problems where a known total + new ratio implies a specific add/remove (e.g., \"the ratio was 3:5, now 4:5; total stayed 80 — what was added to the first part?\"). Distractors compute under a two-part-ratio assumption or against the wrong total.",
	"numerical.workrate":
