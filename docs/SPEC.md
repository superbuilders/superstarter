...
**Sub-pattern note on instance granularity (added 2026-05-09 at Round 2 round-close).** §6.14.40 sub-pattern instances accrue at multiple granularities. **Plan-doc-section level:** Round 1's 5 instances (§0.12, §0.13, §0.14, §5.8.a, §5.8.b) were each captured as discrete `§0.X` audit-finding entries — promotion-justifying instances. **Audit-step level:** Round 2 caught 3 redirector-vs-empirical-state divergences mid-audit (SF-A bifurcated tokens at §0.3; §A.4.f1 supersession at §0.14; strategy-selection cascade at §0.15) where the audit's first action — empirical-state capture — surfaced the divergence and the redirect was corrected before code shipped; not always escalated to a separate plan-doc §0.X entry. **Line-number level:** fine-grained drift caught between authoring and shipping (e.g., commit 1 of Round 2 caught a `globals.css` line-number drift 36-46 → 43-54 + a Layer-B inventory drift mid-commit). Audit-step-granularity + line-number-granularity instances are caught by the rule's pre-flight enforcement; they reinforce the rule's value but are not separately promoted (instance counts at the plan-doc-section granularity stay canonical for promotion-threshold accounting). Round 2's 3 audit-step instances + Round 1's 5 plan-doc-section instances reinforce the parent rule across both Rounds without new sub-rule promotion.
...
#### 6.14.42 Audit-step grep-verify-consumers when deleting/renaming type-or-function exports

> **Captured 2026-05-09** (post-session audit fixes + wide token retrofit round close, Round 2). Single round-2 instance (§5.4 commit-5 audit-step (c) STOP-AND-REPORT halt) but structurally novel + adversarial impact-shape — sister to §6.14.41's single-instance promotion. Promoted on impact-shape rather than instance-count.

When a commit deletes or renames a type, function, or module-level export, the audit step's pre-flight MUST grep across `src/` for consumers (imports + call sites). Cite-without-grep — assuming consumer scope is bounded by the commit's named files — is the analog to §6.14.41's adversarial pattern: when downstream consumers exist elsewhere, face-value execution silently breaks them at compile- or runtime.

**Round-2 empirical anchor** (1 instance):

- Commit 5 halt — combined `<PerformanceSummary>` per Round 2 §5.4 (Option 4 split prep). The redirect framed `<AccuracySummary>` + `<LatencySummary>` deletion as bounded — only the page-level callers + the to-be-deleted components themselves consumed the `PerSubTypeAccuracy` / `PerSubTypeLatency` types. Audit-step (c) ran `grep -rE "PerSubTypeAccuracy|PerSubTypeLatency" src/` as the canonical pre-flight; the grep surfaced **`src/server/post-session/strategy-selection.ts`** (228 lines, 4 public functions taking the per-axis arrays as separate params, 2 page-level call sites — a major server-side consumer the redirect did not anticipate). The audit-step STOPPED-AND-REPORTED before implementation; redirect resolved as **Option 4 split** (commit 5 ships the combined component + SQL consolidation + transient projection shims; commit 5b refactors `strategy-selection.ts` to consume the consolidated shape + retires the shims + retires the per-axis types). Without the audit-step grep, commit 5 would have shipped a deletion that silently broke `strategy-selection.ts`'s type imports + downstream `surfacedStrategies` derivation.

**The failure-mode shape.** Adversarial-direction (sister to §6.14.41): when a deletion/rename is taken at face-value scope, consumers outside the named-files boundary go unnoticed; the typecheck catch eventually surfaces the cascade but the commit envelope balloons mid-implementation, sometimes too late for clean Option-split resolution. Caught at commit-time pre-flight, the cascade scope expands cleanly into bounded sub-commits (e.g., Round 2 §5.4 + §5.4b Option 4 split: each commit's blast radius bounded; transient shim region documented in commit 5's body; cascade resolved end-to-end at commit 5b without cross-commit churn).

**The discipline rule.** Audit-step pre-flight for ANY commit that deletes or renames a type, function, or module-level export MUST run `grep -rE "<NameA>|<NameB>|..." src/` (across all the deletion/rename targets) and capture the consumer list. The grep result MUST appear in the audit-step prose; cite-without-grep is the adversarial-direction §6.14.28 anti-pattern. If the consumer list extends beyond the redirect's framed scope, STOP-AND-REPORT with the surface inventory + scope-expansion options (Option 1 absorb / Option 4 split / Option N-other). The redirect's revised guidance lands on the right Option; commit ships with explicit scope boundary.

**Convention going forward.** Round-opening redline + every commit's audit step list deletion/rename targets explicitly. The audit step's grep enumerates consumers. If consumers exist outside the named-files boundary, the redirect adjusts before code ships; if no extra consumers, the commit ships at audit-time scope. The grep is cheap (sub-second on `src/` of this size); skipping it to "save time" is the false-economy that triggers the adversarial-direction failure.

**Cross-references.** Round empirical instance: §0.15 (Option 4 split discovery — commit 5 audit-step (c) halt; redirect resolution to split into §5.4 + §5.4b) + plan-doc `docs/plans/post-session-audit-fixes-and-wide-token-retrofit.md` §0.15 + §5.4 + §5.4b. Sibling: §6.14.40 (redirector-vs-empirical-state divergence) — §6.14.42's specialization to type-or-function-export deletion is the consumer-scope-discovery analog of §6.14.40's audit-vs-redirect-state-discovery. §6.14.41 (audit-vs-revert blindness) — sister entry with same single-instance + impact-shape-based promotion model; both adversarial-direction. §6.14.30 (additive-feature-cascade-undercount) — parent pattern; §6.14.42 is the deletion-side specialization (the audit step that PREVENTS the cascade undercount from propagating to ship-time). §6.14.21 + §6.14.22 + §6.14.23 — "audit against actual artifact, not assumed shape" parent discipline; §6.14.42 specializes to consumer-graph verification.
...
- **The recency-excluded set AND the within-session-attempted set are SOFT preferences, not hard guarantees.** When the fresh + recency-soft passes both exhaust under the per-session bank size, the session-soft fallback CAN serve EITHER a recency-excluded item OR a within-session-attempted item at the requested tier, rather than force the engine to throw `null`. `metadata_json.fallback_level === 'session-soft'` is the observable marker; consumers asserting session-uniqueness or recency-uniqueness MUST count session-soft fallback rows separately. Verification scenarios that assert "no recency-excluded item is served" or "no within-session re-serve occurs" must either (a) ensure the bank is large enough that fresh/recency-soft never exhaust for the session shape under test, or (b) assert the relaxed invariant: `(distinct items served) + (session-soft fallback rows) === (session length)`. Bank-pressure-driven session-soft fallback is structurally inevitable under fixed-shape session generation when bank cells are smaller than per-session tier demand (e.g., decile-5 brutal-tier slots exceed brutal-tier bank size); growing the bank in pressure cells reduces but cannot eliminate the structural possibility.
...
> **Footnote on chart palette + dark `--sidebar-primary` exception (added 2026-05-09 at Round 2 round-close, post-§5.2 Layer-A retrofit).** The chart palette tokens (`--chart-1` through `--chart-5`, lines 103-107 of `globals.css`) and dark-mode `--sidebar-primary` (line 168 of `globals.css`) are pre-existing chromatic — chroma 0.105-0.245, hue 251-265 — shipped that way as part of shadcn's blue-chart palette + sidebar-accent treatment. They are listed in §13.4.1's Layer-A inventory as `:root`-defined foundation tokens (count of 31 includes them), but they are **NOT subject to the Layer-A grayscale-baseline retrofit semantic** introduced at Round 2 §5.2 — they ship pre-tinted by upstream shadcn intent, not by Round 2's hue-270 chroma 0.005-0.012 retrofit. **When evaluating future Layer-A retrofits, exclude these tokens from chroma-bump operations.** Surfaced empirically at Round 2 commit 2's audit-step (a) finding (per `docs/plans/post-session-audit-fixes-and-wide-token-retrofit.md` §5.2 §6.14.28 commit-time addendum Finding 1); commit 2 retrofitted 26 of the 31 Layer-A tokens × 2 modes = 52 token-mode pairs (NOT 31 × 2 = 62), with chart palette + dark `--sidebar-primary` left at their pre-existing chromatic values.
