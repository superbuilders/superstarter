...
> **Phase 5 sub-phase 3 (full-length test) opens 2026-05-07.** This sub-phase is Phase 5 v1's last unshipped sub-phase; once it closes, Phase 5 v1 is feature-complete and the no-deploy-until-feature-complete gate per the master plan lifts. Predecessors all shipped: sub-phase 1 (post-session review surface, 2026-05-04), sub-phase 2 (adaptive walker, 2026-05-06), sub-phase 5 (dojo UI rename + belt indicator, 2026-05-06), sub-phase 4 (click-to-highlight in post-session explanation review, 2026-05-07). The route pair is `/full-length/configure` + `/full-length/run` (NOT `/test/...` — the historical `/test/page.tsx` framing in item 1 below predates the sub-phase 3 plan and reconciles past-tense at round close). The cross-sub-type-interleaved 50-slot decile generator lives in `src/config/difficulty-curves.ts` alongside the existing `standardCurve` + `roundDecile`. Full-length post-session inherits the existing `<PostSessionShell>` non-diagnostic render path with zero new slots and zero new gates — the slot-1 belt indicator stays drill-only, slots 7-8 stay diagnostic-only, slot 9 (Continue CTA) renders. The post-session heading stays "Session complete" (drill-shared); no full-length-specific copy. See `docs/plans/phase5-full-length-test.md` for the round; past-tense reconciliation lands at the round-close commit.
...
1. ~~`/test/page.tsx` renders `<NarrowingRamp>`.~~ **Cut 2026-05-04.** v1 `/full-length/configure/page.tsx` renders a bare primer pane (no length-picker, no sub-type-picker — the test is fixed at 50q × 15min cross-sub-type-interleaved); the configure page's submit navigates to `/full-length/run/page.tsx`, which calls `startSession({ type: "full_length" })` directly (no `ifThenPlan`).
