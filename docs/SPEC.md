...
5. `<WrongItemsBrowser>` — sub-type grouped (verbal-first, alphabetical within section), chronological within group via `attempts.id` ASC, prose explanation only. Display letters A-E computed via `String.fromCharCode(0x41 + index)` at render time per SPEC §3.3.2 — letters are NOT stored. **Click-to-highlight extension in flight (Phase 5 sub-phase 4):** the per-card explanation render swaps from prose to structured parts (recognition / elimination / [optional] tie-breaker) sourced from `metadata_json.structuredExplanation` per §3.3.3, with elimination clicks striking through the options the part references via `referencedOptions` and tie-breaker clicks highlighting the options it considers. State is per-card, ephemeral, reset per session render. The 50 NULL-`source_folder` seed items lack `structuredExplanation` and render the prose form unchanged via a silent fallback. See `docs/plans/phase5-click-to-highlight.md` for the round; past-tense reconciliation lands at the round-close commit.
