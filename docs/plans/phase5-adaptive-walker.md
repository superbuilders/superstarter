...
> **Status: shipped 2026-05-06.** The four-commit sequence below landed on `main` cleanly:
>
>   - Commit 1 (walker pure function + unit tests, dormant) — `9a522b1` — `feat(selection): adaptive walker — nextDifficultyTier pure function + unit tests`
>   - Commit 2 (wire walker into drill arm; flip drill from uniform_band to adaptive) — `88335b5` — `feat(selection): wire walker into drill arm; flip drill from uniform_band to adaptive`
>   - Commit 3 (remove vestigial uniform_band strategy and getNextUniformBand) — `4781ee5` — `chore(selection): remove vestigial uniform_band strategy and getNextUniformBand`
>   - Commit 4 (doc reconciliation + plan close) — *this commit* — `docs(spec+plan): reframe SPEC §9.2 verification clarification post-uniform_band-removal; close phase5-adaptive-walker`
>
> **Round-close summary.** Sub-phase 2 ships the SPEC §9.1 adaptive difficulty walker as the drill arm's tier-decision path. Commit 1 landed `nextDifficultyTier(ctx)` + `stepUp` / `stepDown` as a dormant pure function plus 8 unit tests covering the floor, step-up, step-down, clamp, hold, and strict-< latency-boundary scenarios (38 → 46 tests). Commit 2 wired `getNextAdaptive(ctx)` into the drill arm with the in-session attempt-window read (top-10 rev-chronological by `attempts.id` UUIDv7 PK, session-scoped, no sub-type filter per §9.6), flipped `selectionStrategyForSession("drill")` from `'uniform_band'` to `'adaptive'`, replaced the throw-`ErrAdaptiveDeferred` arm with `return getNextAdaptive(ctx)`, and added 3 integration tests against verbal.antonyms (initial tier from mastery_state, walker holds across the 10-attempt floor, walker steps `easy → medium` after 10 attempts at high performance) (46 → 49 tests). EXPLAIN ANALYZE on the in-session-window query confirmed `Index Scan using attempts_session_id_idx` at 0.087ms execution; a five-scenario throwaway real-DB harness in `/tmp/walker-harness.ts` validated chain step-up (`easy → medium → hard`, `medium → hard → brutal`), step-down by latency (median 18000ms > 1.2× threshold), step-down by accuracy (5/10 → easy with clamp), and hold-in-zone (7/10 + in-zone latency) end-to-end. Commit 3 removed the now-vestigial `'uniform_band'` from the `SelectionStrategy` type union (now `'fixed_curve' | 'adaptive'`), removed `getNextUniformBand` (-44 lines), removed the `'uniform_band'` dispatch arm, removed `ErrAdaptiveDeferred` constant + export, and refreshed narrative comments — including two cascade fixes in `scripts/dev/smoke/phase3-commit5.ts` per §6.14.19 (cascade fix at removal-commit time, not in a follow-up); `rg "uniform_band"` post-commit-3 returned only the file-header sentence documenting the removal, `rg "ErrAdaptiveDeferred"` and `rg "getNextUniformBand"` returned zero hits. Commit 4 (this commit) reframed SPEC §9.2's "no-walking contract on REQUESTED tier" clarification — which had framed its semantic content around `uniform_band`'s no-walking contract — to a "verification reads REQUESTED tier, not served tier" framing that drops the dead `uniform_band` reference while preserving the verification-scenarios guidance that was the clarification's load-bearing content. SPEC §9.1 needed no reframe (it was already SPEC-complete; the implementation caught up to the contract). Test count holds at 49 across commit 3 + 4. The dev DB's 14 × 4 = 56-tier-cell snapshot at commit 2 close: 44 populated cells, 12 zero cells (10 brutal-tier across most sub-types — bank not yet seeded for brutal, walker handles via tier-degraded fallback; 2 carryover from the testbank-re-extraction round close at `numerical.workrate.easy` and `numerical.lowest_values.hard`, both handled by the existing fallback chain). No new zero cells emerged during the round. Going into sub-phase 5 (dojo belt indicator) planning, the codebase is now cleanly sub-phase-2-complete: drill mode runs the adaptive walker live, the walker exports `nextDifficultyTier` / `AdaptiveContext` / `Tier` for downstream consumers (sub-phase 5 will visualize the walker's current-tier output), the `'adaptive'` arm of `getNextItem` is the only drill-side dispatch path, and the SPEC §9.2 verification-scenarios clarification reads cleanly against the walker.

> **Drafted audit-first against `main` post-diagnostic-bug-fixes-round close (HEAD = `a9df0f6` at draft time).** The master plan's §4 framing is the starting point; where the audit recommends a different shape, the plan recommends that shape with rationale.
