...
- **SF-B `<MasteryMap>` referenced but absent:** **RESOLVED at commit 2 pre-flight** (per §5.2 audit step (c); grep confirms current state OR flags as stale-prose for separate cleanup).
...
6. **SF-B grep follow-up (resolves at commit 2 pre-flight).** If `<MasteryMap>` reference is stale-prose, separate cleanup commit (not Round 2-blocking).
