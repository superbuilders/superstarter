...
>
>   - Commit 1 (selection.ts + start.ts) — `7bc96ea` — `refactor(sessions): drop review-queue strategy + vestigial timer_mode branches + ifThenPlan path; align spec`
>   - Commit 2 (focus-shell internals) — `a32131a` — `refactor(focus-shell): drop timerPrefs state + ifThenPlan prop chain; align spec`
>   - Commit 3 (schema column drops + enum truncations) — `938f771` — `refactor(schema): drop vestigial columns + truncate timer_mode/session_type enums; align spec`
>   - Commit 4 (schema table drops + barrel cleanup) — `37ad762` — `refactor(schema): drop review_queue + strategy_views tables; align spec`
>   - Commit 5 (doc reconciliation + plan close) — *this commit* — `docs(spec+prd+plan): reconcile cut markers to past-tense; add §6.14.17; close v1-code-cleanup round`
>
> **Round-close summary.** The doc-only v1-scope-tightening round (`ca2330a` → `832f634`) marked five v1 cuts (spaced-repetition queue, strategy-review gate, speed-ramp + brutal drill modes, timer-toggle UX, NarrowingRamp protocol) without corresponding code excision; this code-cleanup round excised the vestigial code surfaces those markers described. The five commits sequenced leaf-first: server-side throwing-stub deletion (commit 1) → focus-shell internal dead-state removal (commit 2) → schema column drops + enum truncations via Drizzle migration `0001_true_young_avengers.sql` (commit 3) → schema table drops via migration `0002_tranquil_mach_iv.sql` (commit 4) → SPEC + PRD doc reconciliation + plan close (this commit). The original 6-commit shape included an application-layer-barrel slot between commits 2 and the schema work; the audit at commit-2-close confirmed Outcome 1 (commits 1 + 2 rippled cleanly), so the slot was dropped and the round renumbered to 5 commits atomically with commit 3's migration work (see §12.5). Going into Phase 5 sub-phase 1 planning, the codebase is now legibly v1-clean: `selection.ts` carries no vestigial throwing stubs or dead `timer_mode` branches; the focus shell's reducer state shape matches its v1 visual contract; the database schema reflects only what v1 writes (no `timer_prefs_json`, `narrowing_ramp_completed`, `if_then_plan`, `strategy_review_viewed`; no `review_queue` or `strategy_views` tables; `timer_mode` enum truncated to `['standard']`; `session_type` enum truncated to four values). The doc-only round's cut markers were tightened to past-tense across PRD + SPEC; SPEC §6.14.17 (added in this commit) captures the doc-only-cut-from-v1-reconciliation pattern + the inversion-during-doc-only-reconciliation pattern caught during commit 2's §6.12 audio-silent correction, so future scope-tightening rounds inherit the discipline.

This round drops the vestigial code surfaces left behind by the 2026-05-04 doc-only v1-scope-tightening (commits `ca2330a` / `991d3eb` / `75240c8` / `4b35449` / `141bf83` + the hygiene follow-up `832f634`). The doc-only round established what's cut and what stays vestigial in tree; this round excised the vestigial code so it doesn't leak into Phase 5 sub-phase 1's audit findings. **Five commits**, all small, sequenced leaf-first — original commit 3 (application-layer barrel + drill route signature alignment) was dropped after the audit pass at commit-2-close confirmed Outcome 1 (commits 1 + 2 rippled cleanly; no application-layer cleanup needed). Renumber landed atomically with new commit 3's migration work. Per-commit implementation detail (specific migration SQL, exact reducer-state diffs) lives in the commits themselves; this plan is sequencing-shaped.
