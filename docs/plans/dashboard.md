...
>
>   - Commit 1 (plan-doc redlines + design tokens + `@theme inline` + font swap PJS+Newsreader + `metadata.title` "Todos"→"18seconds") — `5cb61dd` — `feat(dashboard): plan-doc redlines + design tokens + font swap + metadata.title`
>   - Commit 2 (`user_sub_type_belts` stub schema + barrel registration; deferred migration per PRD §16) — `ff93fe1` — `feat(dashboard): user_sub_type_belts stub schema + barrel registration`
>   - Commit 3 (Mastery Map page-mount migration `(app)/page.tsx` → `(app)/drill/page.tsx`; `<EmptyBankPane>` href rewrite `/` → `/drill`) — `f4d0653` — `feat(dashboard): migrate Mastery Map to /drill index`
>   - Commit 4 (stub pages for `/lessons`, `/stats`, `/review` so the to-be-built TopNav has zero 404 targets) — `ab4033c` — `feat(dashboard): stub pages for /lessons, /stats, /review`
>   - Commit 5 (server data layer: types.ts + helpers.ts + 6 stubbed helpers + 1 real `loadUserProfile` + orchestrator `getDashboardData`; decisions E/F/G land as code) — `a8e8ca9` — `feat(dashboard): server data layer (types, helpers, seven stubs, orchestrator)`
>   - Commit 6 (leaf components: `<BeltStripe>` + `<StatTile>` + `<StreakChip>`; `bg-bg` Tailwind v4 utility-generation verified — plan §7 risk did not materialize) — `d38b19b` — `feat(dashboard): leaf components — BeltStripe, StatTile, StreakChip`
>   - Commit 7 (composite components: `<BeltRow>` + `<DojoCard>` + `<MissionCard>`; first PRD-vs-project-rule reconciliation surfaced — inline-style for dynamic widths refactored to SVG `<rect>` per latency-summary precedent; `<Link href={dynamic-string}>` refactored to plain `<a>` per typedRoutes constraint) — `befe9bd` — `feat(dashboard): composite components — BeltRow, DojoCard, MissionCard`
>   - Commit 8 (bottom-row composites: `<ScoreStrip>` + `<PaceMetric>` + `<MistakesTile>` + `<LastSimTile>`; nested-const-ternary lint surprise refactored to early-return helper; PaceMetric SVG-bar refactor mirroring commit 7's pattern) — `3abe143` — `feat(dashboard): bottom-row composites — ScoreStrip, PaceMetric, MistakesTile, LastSimTile`
>   - Commit 9 (nav + client wrapper: `<TopNav>` + `<Dashboard>`; first `"use client"` introductions of the round; usePathname runtime-vs-type drift caught by streaming-SSR throwaway harness — the .d.ts claims `string` but SSR returns `null`) — `49894d7` — `feat(dashboard): nav + client wrapper — TopNav, Dashboard`
>   - Commit 10 (page mount: `(app)/page.tsx` content REPLACED with the dashboard server component; net −74 lines because the dashboard's single-orchestrator data shape replaces the Mastery Map's 4+-promise composition) — `ad95e9b` — `feat(dashboard): page mount — replace (app)/page.tsx with the dashboard server component`
>   - Commit 11 (full-surface Alpha Style audit + polish; PRD §15 32/32 invariants verified, 31 statically + 1 WCAG AA deferred to authenticated DevTools pass; `prefers-reduced-motion` rule added per commit-1 audit-surfaced finding; `--dashboard-belt-*` namespace fork NOT applied per PRD §8 audit recommendation 1 default) — `88417f8` — `feat(dashboard): full-surface Alpha Style audit + polish`
>   - Commit 12 (round-close — plan-doc status flip) — *this commit* — `docs(plan): round-close — plan-doc status flip`
>
> **Round-close summary.** The dashboard round ships the "Dojo" dashboard at `/` as the first post-Phase-5-v1 feature surface. End-to-end render is live: `(app)/layout.tsx` gates auth + diagnostic-completed → `(app)/page.tsx` (server component) initiates a `getDashboardData(userId)` promise, threads it through `<React.Suspense>` to `<Dashboard>` (client component) which `React.use()`s it and renders the 8-component composition (TopNav + ScoreStrip + MissionCard + 2× DojoCard + PaceMetric + MistakesTile + LastSimTile). The Mastery Map content migrated to `(app)/drill/page.tsx` at commit 3 (2-step pattern: copy → replace, kept `/` a stable known-good surface during commits 3-9); stub pages at `/lessons`, `/stats`, `/review` ship at commit 4 so the TopNav's 5 routes all resolve to a 200. The data layer (commit 5) is 1 real read (`loadUserProfile` against `users`, with decision G's null-name validate-and-throw + STUB_GOAL_SCORE=40 inline stub) plus 7 stubs returning deterministic empty values that drive the dashboard's empty-state branches (`current/daysToTest = "—"`, `streakDays = 0` → "Start your streak", `lastSim = undefined` → "Take your first sim →", `count = 0` → "No mistakes to review"); each stub names its follow-up PRD per PRD §19's stub-removal sequence. The 12 components under `src/components/dashboard/` consume ~30 new dashboard tokens added at commit 1 (`--bg`, `--surface`, `--surface-2`, `--border-soft`, `--border-strong`, `--text-1/2/3`, `--cobalt`, `--indigo`, `--alpha-accent`, `--lavender`, `--lavender-line`, `--belt-white/-line`, `--belt-black`, `--pace-on/warn/over`, `--good`, type-scale, spacing, radii, motion durations) plus the existing sub-phase-5 `--belt-blue`/`--belt-brown` (untouched per PRD §8 audit-recommendation-1 default — the post-session `<BeltIndicator>` at `b31d8cb` retains bit-for-bit token preservation). The `user_sub_type_belts` Drizzle schema lands at commit 2 with `db:generate`/`db:push` deferred to the future Belts PRD; the dashboard's `loadAllBelts` helper stubs to all-white. Two PRD-vs-project-rule reconciliations applied at commit-time (commits 7 + 8): (a) PRD listings used `style={{ width/height: \`${pct}%\` }}` for dynamic dimensions on BeltRow's progress bar and PaceMetric's seven bars; gritql/no-inline-style.grit bans inline styles, refactored to SVG `<rect>` with dynamic `width`/`height` JSX attributes per `src/components/post-session/latency-summary.tsx` precedent; (b) PRD listings used `<Link href={dynamicString}>` for BeltRow row hrefs + MissionCard CTAs + MistakesTile + LastSimTile; `next.config.ts` has `typedRoutes: true` which requires Route-literal hrefs for `<Link>`, refactored to plain `<a>` per `src/components/mastery-map/start-session-button.tsx` precedent (loss of soft-nav prefetch accepted in exchange for type-safety + rule-compliance). One audit-surfaced finding from commit 1 landed at commit 11: PRD §8 prose claimed `prefers-reduced-motion: reduce` was already in `globals.css`; git-log confirmed it never was, added per WCAG 2.3.3 + ALPHA §6 with `!important` properties (each marked with biome-ignore-line documenting WCAG-canonical justification). Tests held at the 79/79 invariant across all 11 prior commits' stable runs; one pre-existing probabilistic flake at `src/server/items/selection.test.ts:684` (`fullLengthNoReServe` test, ~10-20% rate, bank-depth-sensitive on thin sub-type cells) surfaced intermittently across commits 2/4/5/7/8/10 but was never introduced by any round commit (every dashboard commit's edits are confined to `src/server/dashboard/`, `src/components/dashboard/`, `src/styles/unstyled/globals.css`, `src/app/layout.tsx`, `src/app/(app)/{page,drill/page,lessons/page,review/page,stats/page}.tsx`, `src/db/schema.ts`, `src/db/schemas/practice/user-sub-type-belts.ts`, or `docs/plans/dashboard.md`). PRD §15 acceptance: 32/32 verified — 31 statically via streaming-SSR throwaway harnesses + lint/typecheck/test invariants + compiled-CSS-bundle inspection, 1 (WCAG AA contrast) deferred to authenticated DevTools pass per the auth-gated-curl constraint flagged at commits 3+10. Decisions E (`type IN ('full_length', 'simulation')` comment in score.ts), F (Title Case `s.displayName` in belts.ts), G (validate-and-throw on null `users.name` in data.ts), H (`metadata.title` "18seconds"), I (Leo eyeballs deferred to commit 11 — no flags) all resolved + landed as code. The 9-step stub-removal sequence per PRD §19 (Sim Scoring → Pace-Strip → Belts → Goal Score → Mission Picker → Streaks → Mistakes → Lessons → Stats) is intact and unblocked; none of the nine follow-up PRDs depend on each other or on this round's deliverable beyond the data-contract types in `@/server/dashboard/types`. Five §6.14 candidates accumulated across the round (PRD-prose-vs-git-history mismatch at commit 1, no-inline-ternary nested-const grit-rule scope drift at commit 8, usePathname .d.ts-vs-runtime drift at commit 9, two-step migration pattern at commit 10, blanket-vs-scoped reduced-motion at commit 11); promotion to SPEC §6.14 entries is INTENTIONALLY DEFERRED to a separate post-round operational sub-round (doc-only, ~1 commit) per the round-close brief — this commit is plan-doc-only. The dashboard round is feature-complete and ready for whatever Leo decides next: deploy + dogfood, post-round §6.14 promotion sub-round, or one of the nine PRD §19 follow-up PRDs.
