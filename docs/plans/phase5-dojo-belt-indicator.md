...
> **Status: shipped 2026-05-06.** The six-commit sequence below landed on `main` cleanly:
>
>   - Commit 1 (round-open docs) — `8fc6957` — `docs(prd+spec): open phase5-dojo-belt-indicator round; PRD §4.2 + SPEC §10.2 + §10.7 dojo + belt-indicator narrative`
>   - Commit 2 (`getEndSessionTierForDrill` query + types, dormant) — `b53d2c2` — `feat(post-session): getEndSessionTierForDrill query + types`
>   - Commit 3 (`<BeltIndicator>` component + `tierToBeltColor` helper, dormant) — `b31d8cb` — `feat(post-session): BeltIndicator component + tier-color mapping`
>   - Commit 4 (wire BeltIndicator into shell heading; drill heading expansion) — `c3c5a88` — `feat(post-session): wire BeltIndicator into shell heading; drill heading expansion`
>   - Commit 5 (dojo rename across drill route + Mastery Map CTA + run skeleton; full-surface audit + polish) — `c32a7fb` — `feat(ui): dojo rename across drill route + Mastery Map CTA + run skeleton; full-surface audit + polish`
>   - Commit 6 (round-close doc reconciliation) — *this commit* — `docs(spec+plan): reconcile §10.2 + §10.7 to past-tense; close phase5-dojo-belt-indicator round`
>
> **Round-close summary.** Sub-phase 5 ships the dojo UI rename + adaptive-walker belt indicator on the drill-mode post-session shell, completing Phase 5 v1's user-facing UX shift. The dojo metaphor lands end-to-end: Mastery Map "Enter dojo: {sub-type}" → drill configure subhead "Standard timing. Pick a session length and enter the dojo." + submit "Enter dojo" → drill run skeleton "Preparing your dojo session…" → post-session `<BeltIndicator>` rendering the walker's session-end tier as a belt-shaped SVG ("you reached the {color} belt on {sub-type}", with "(calibrating)" suffix when attempt count < `ADAPTIVE_FLOOR_ATTEMPTS=10`). Belt-color → tier mapping settled at the 4-color compressed shape per plan §11.2 (white = easy, blue = medium, brown = hard, black = brutal) — the master plan's 6-color sketch was over-determined for the walker's 4 tiers; compressing keeps each color's signal high and matches the walker's tier domain exactly. Component-render slot was the heading area (slot 1 expansion per plan §11.7), preserving the post-session shell's locked nine-slot ordering bit-for-bit; commit 4's atomic wiring across page query → content prop → shell heading-area means the belt only renders for drill mode + non-null tier (zero-attempt sessions render the heading unchanged per plan §5.5). Two net-new design tokens (`--belt-blue`, `--belt-brown`) were added to `src/styles/unstyled/globals.css` per plan §6.3's below-3-systemic-token framework — both belt-namespaced + single-occurrence; the audit-against-actual-artifact pattern (SPEC §6.14.18) corrected plan §5.2's "established Alpha Style accent blue" framing (no such structural token existed in the codebase). White and black belts reuse `--card` and `--foreground` respectively. Test count grew 49 → 60 across commits 2 + 3 (5 integration tests on the query + 6 unit tests on the helpers); commit 4's 5-scenario real-DB harness + Playwright headless visual spot-check provided the component-render integration verification per plan §9, since the codebase has no component-test infrastructure (audit-against-actual-artifact finding from commit 3 — plan §7.1 referenced "infrastructure already established for sub-phase 1's components" but no such infrastructure exists in tree). Commit 5's full-surface Alpha Style audit per plan §6.2 was clean across the post-session shell + drill route surfaces; polish was a no-op. EXPLAIN ANALYZE on the new query (commit 2) ran sub-2ms at v1 attempts-table scale (2204 attempts) using `attempts_session_id_idx` on the outer join + `attempts_pkey` backward scan on the correlated subquery — both acceptable per SPEC §6.14.13 dev-vs-prod planner choice. The `ADAPTIVE_FLOOR_ATTEMPTS=10` walker constant was extracted from `src/server/items/selection.ts` and exported alongside `nextDifficultyTier` so the belt indicator's pre-floor branch can reference the same threshold; walker behavior unchanged. Bank totals invariant 439 / 50 / 389 throughout. Going into Phase 5 v1 round close: sub-phase 5 is complete; sub-phases 3 (full-length test) and 4 (click-to-highlight in wrong-items browser) remain unshipped but are independent of sub-phase 5 and can ship in any order.
