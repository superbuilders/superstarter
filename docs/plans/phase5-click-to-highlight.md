...
> **Status: shipped 2026-05-07.** The five-commit sequence below landed on `main` cleanly:
>
>   - Commit 1 (round-open docs) — `4943f52` — `docs(prd+spec): open phase5-click-to-highlight round; PRD §6.5 click-to-highlight bullet + SPEC §3.3.3 + §10.7 narrative`
>   - Commit 2 (page-query projection + `WrongItem.structuredExplanation` field, dormant) — `285fee6` — `feat(post-session): WrongItem.structuredExplanation field + page-query projection`
>   - Commit 3 (`<StructuredExplanation>` component + Zod parse boundary + 9 helper unit tests, dormant) — `2b65d01` — `feat(post-session): <StructuredExplanation> component + Zod parse boundary`
>   - Commit 4 (atomic dormancy unblock — `<WrongItemCard>` state + `<OptionLine>` strike/highlight) — `9fdc893` — `feat(post-session): wire <StructuredExplanation> into <WrongItemCard>; <OptionLine> strike/highlight effects`
>   - Commit 5 (round-close doc reconciliation) — *this commit* — `docs(spec+prd+plan): full-surface audit + polish; reconcile §10.7 + §3.3.3 to past-tense; close phase5-click-to-highlight round`
>
> **Round-close summary.** Sub-phase 4 ships click-to-highlight on the drill-mode post-session `<WrongItemsBrowser>`'s per-card explanation render, closing the §15.2-amendment seam that sub-phase 1 carved on 2026-05-04 and the master plan §6's last-remaining v1 UX commitment. The seam shipped atomically across three dormant commits + one wire-up: commit 2 added the `metadata_json -> 'structuredExplanation'` projection to `getWrongItemsForSession` plus the matching `WrongItem.structuredExplanation?: unknown` field (with manual `null → undefined` boundary normalize matching the existing `selectedAnswer` / `explanation` convention; EXPLAIN ANALYZE sub-2ms at v1 scale, same plan shape as pre-edit modulo column projection); commit 3 added `<StructuredExplanation>` (`src/components/post-session/structured-explanation.tsx`) with a third copy of the `structuredExplanation` Zod schema (locally co-located per plan §5.2 + §11.10 — the centralization of the three copies across `src/server/items/ingest.ts` + `scripts/_lib/explain.ts` + this renderer is a standing round candidate); commit 4 wired the consumer (atomic single-commit unblock) — `<WrongItemCard>` gained two `useState<ReadonlyArray<string>>` slots (`struck`, `highlighted`), `<OptionLine>` gained optional `isStruck` / `isHighlighted` props, and a three-branch conditional explanation render (structured → `<StructuredExplanation>` ; prose-only → `<p>` ; both absent → null). 389 of 439 live items (88.6%) carry `structuredExplanation`; the remaining 50 NULL-`source_folder` pre-round seed items render the prose fallback silently per plan §3.6 + §11.7 — backfill stays a standing round candidate per Q11.3. Test count grew 60 → 69 (+9 unit tests on the `parseStructuredExplanation` + `ariaLabelFor*` + `classForInteractive` pure helpers; component-render integration via real-DB harness + Playwright spot-check per plan §7.2 — 7 / 7 scenarios pass against session `019dfea6-4d16-702f-b994-d8a3fd05b5ae` which carries the mixed shape — 4 structured cards including 1 three-part with tie-breaker + 1 prose-only fallback). Visual treatment settled at commit 4: `bg-foreground/5 + ring-1 ring-foreground/15` for `<OptionLine>` `isHighlighted`; `line-through + text-foreground/60` for `isStruck`; `<StructuredExplanation>` active part reuses `bg-foreground/5 + ring-1 ring-foreground/15`. The `bg-foreground/5` count grew to 3 across the post-session shell (correct-option marker + StructuredExplanation active + isHighlighted) — at the threshold but **semantically aligned** ("active emphasis" in all three uses; disambiguation layered via font-medium / ring / ✓ marker), so no namespaced token earned promotion per plan §11.5 + the `.alpha-style.md` below-3-systemic-token framework's inconsistency-trigger criterion. Recognition parts are uniformly non-interactive regardless of `referencedOptions` length per plan §3.1 + §11.9 — even the audit-surfaced minority of items with non-empty recognition refs (1 or 4 refs in the bank sample) renders as plain `<p>` to keep the UX simple. Mobile/touch interaction stays at tap=click only per plan §3.5 + §11.6; no long-press / swipe / double-tap. Audit-against-actual-artifact (SPEC §6.14.18) corrected three plan-time framings during implementation: (a) commit 3's `parseStructuredExplanation` had to extract narrowed locals (`recognition`, `elimination`, `tieBreaker`) after the null-check because TypeScript closure narrowing on `parsed` was lost across the `useState`-bound toggle handlers; (b) `rules/no-inline-ternary` flags multi-line const-assignment ternaries despite the rule doc nominally allowing them — both commit 2's normalize-map and commit 3's toggle handlers got rewritten with explicit `if/else` (cleaner anyway); (c) commit 4's first-pass real-DB harness assumed the first DOM-order `<WrongItemCard>` owned the first elimination button — false because the prose-only seed item happened to sit in the first sub-type group (verbal-first, alphabetical), so the harness had to walk from the elimination button up to its actual parent card via xpath ancestor-axis lookup. Production wire-up was correct from the first edit; only the harness's identity assumption was wrong. Bank totals invariant 439 / 50 / 389 throughout. Going into Phase 5 v1 round close: sub-phase 4 is complete; sub-phase 3 (full-length test) remains as the last unshipped Phase 5 v1 sub-phase — independent of sub-phase 4 and ready to ship in any order.
