...
> **Status: shipped 2026-05-07.** The six-commit sequence below landed on `main` cleanly:
>
>   - Commit 1 (round-open docs) — `bc7e4be` — `docs(spec): open phase5-full-length-test round; SPEC §10.3 forward-looking adjustments`
>   - Commit 2 (slot generator + 7 unit tests, dormant; PRNG centralization via export-add to `diagnostic-mix.ts`) — `a7ec40a` — `feat(config): full-length-slot generator + per-session deterministic interleaving`
>   - Commit 3 (engine dispatch wired into `getNextFixedCurve`; 3 integration tests against tier-degraded fallback + slot-by-slot integrity + no-re-serve-in-session) — `74f2dac` — `feat(selection): wire full-length into getNextFixedCurve`
>   - Commit 4 (atomic dormancy unblock — `/full-length/configure` + `/full-length/run` routes + Mastery Map secondary CTA; end-to-end full-length flow becomes reachable in browser) — `61a650c` — `feat(app): /full-length/configure + /full-length/run routes; Mastery Map secondary CTA`
>   - Commit 5 (full-surface Alpha Style audit + polish; clean across all three surfaces; EXPLAIN ANALYZE re-runs against full-length fixture session captured for the v1-feature-complete record) — `9376423` — `feat(post-session): full-surface audit + polish across full-length post-session render path`
>   - Commit 6 (round-close doc reconciliation) — *this commit* — `docs(spec+plan): reconcile §10.3 + §10.7 to past-tense; close phase5-full-length-test round`
>
> **Round-close summary.** Sub-phase 3 ships the SPEC §10.3 / PRD §4.5 full-length practice test as Phase 5 v1's last user-facing surface. End-to-end flow is live: Mastery Map "Take a full-length test" secondary CTA → `/full-length/configure` bare primer pane → submit → `/full-length/run` mounts `<FocusShell>` with `sessionDurationMs: 900_000` + `perQuestionTargetMs: 18_000` + `paceTrackVisible: true` + `targetQuestionCount: 50` + `strictMode: false` → 50-attempt completion OR 15-minute auto-end → `/post-session/[sessionId]` (drill-shared "Session complete" heading; slot 1 belt indicator null per drill-only guard; slots 7-8 null per diagnostic-only guards; slot 9 Continue CTA renders; slots 2-6 type-agnostic and render unchanged including sub-phase 4's click-to-highlight on wrong items). The cross-sub-type-interleaved 50-slot decile generator (`generateFullLengthSlots(sessionId)`) lives in `src/config/difficulty-curves.ts` next to the existing `standardCurve` + `roundDecile` per Q12.1 — pure function, per-session deterministic via xmur3+mulberry32 PRNG seeded `${sessionId}:d${decileIndex}:${tier}` for tier-pool draws and `${sessionId}:d${decileIndex}:order` for Fisher-Yates per-decile shuffle; uniform sub-type draws-with-replacement per Q12.3; tuples may repeat across slots (Q12.10) while item ids never repeat in-session (the existing `pickWithFallback` `sessionAttemptedIds` exclusion). Engine dispatch in `getNextFixedCurve` extends the diagnostic-only path with a type-switched if/else-if/else: `diagnostic` → `shuffledDiagnosticOrder`, `full_length` → `generateFullLengthSlots`, simulation/drill → defensive throw. Test count grew 69 → 79 (+7 unit tests on the slot generator's per-decile distribution + per-session determinism + cross-session variation + 14-sub-type union coverage + no-section-break interleaving + per-(sessionId, decileIndex) decoupling; +3 integration tests on engine completion + slot-by-slot integrity + no-re-serve-in-session). The bank's audit-surfaced thin cell `numerical.ratios.hard = 0` (commit 1's 56-tier-cell snapshot) was empirically validated at commit 3 — `pickWithFallback` walks `hard → medium` and records `fallback_level: 'tier-degraded'` + `fallback_from_tier: 'hard'`; integration test 2's `degradedSeen >= 1` assertion fires reliably (~99% per-session probability given the bank's 12 zero cells). Zero session-type-aware edits to existing components per Q12.8 — `post-session-shell.tsx` is bit-for-bit unchanged since `c32a7fb` (sub-phase 5 close), confirming the slot-locking discipline established at sub-phase 1 + sub-phase 5 held through sub-phases 4 + 5's downstream churn. The Mastery Map secondary CTA addition is one anchor inside the existing `flex-col gap-3` wrapper using `text-foreground/60 text-sm underline-offset-4 hover:underline` — visual hierarchy reads correctly as primary `<Button size="lg">` > secondary anchor > peripheral `<TriageAdherenceLine>` (`text-foreground/40 text-xs`). The atomic dormancy chain (commit 2 → 3 → 4) unblocked cleanly at commit 4 across six layers (config → engine dispatch → routes → Mastery Map CTA → live focus-shell flow → post-session shell render). Audit-against-actual-artifact (SPEC §6.14.18) corrected three plan-time framings during implementation: (a) plan §3 sketched `generateFullLengthSlots` as a single function; biome's `noExcessiveCognitiveComplexity` rule flagged the inlined version at 21 (max 15), driving extraction into four pure helpers (`seededRand`, `pickSubTypesWithReplacement`, `shuffleInPlace`, `buildDecileSlots`) for a strict readability + per-helper-testability win; (b) plan §3 implementation seam said xmur3+mulberry32 might already be exported from `diagnostic-mix.ts` — they were locally scoped, so commit 2 added named exports (option (a) per the brief — 1-line edit to the export statement, body untouched); (c) plan §2.8 referenced "SPEC §6.5 (Post-session review composition)" — actual SPEC structure has post-session-review-composition at §10.7 (§6.5 is Latency measurement, unrelated); the round-close updated §10.7 to add a sub-phase-3 marker matching the §10.7 convention used by sub-phases 1, 4, 5. EXPLAIN ANALYZE re-runs at commit 5 against fixture session `019e0363-d0e7-7304-a520-583a4c4ce4ea` confirmed all five post-session aggregations match the diagnostic baseline plan shape (Bitmap/Hash Join + `attempts_session_id_idx`; sub-3ms execution; Seq Scan on items at v1 dev-DB scale per SPEC §6.14.13). Commit 5's `text-foreground/60` codebase-wide grep found 13 occurrences across 7 files (correcting commit 4's claim of count=1) — all semantically aligned as "secondary visual weight"; the token is functioning as a stable systemic primitive, no namespaced token earned promotion. Bank totals invariant 439 / 50 / 389 throughout. Phase 5 v1 is now feature-complete; the no-deploy-until-feature-complete gate per master plan §1 lifts. The post-Phase-5 sequence (cleanup of vestigial v1-cut artifacts; deploy-and-dogfood; operational hardening) is the next planned arc.
